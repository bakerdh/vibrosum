...
if (!is_tinytex()){install_tinytex(force=FALSE)}
...
if (!dir.exists('local')){dir.create('local/rawEEGdata/')}
osfproject <- osf_retrieve_node('z2fcm')
osffiles <- osf_ls_files(osfproject,n_max=300)
for (s in 1:31){
if (!file.exists(paste0('local/rawEEGdata/P',s,'_EEG.fdt'))){osf_download(osffiles[which(osffiles$name==paste0('P',s,'_EEG.fdt')),],'local/rawEEGdata/',progress=TRUE)}
if (!file.exists(paste0('local/rawEEGdata/P',s,'_EEG.set'))){osf_download(osffiles[which(osffiles$name==paste0('P',s,'_EEG.set')),],'local/rawEEGdata/',progress=TRUE)}
}
}
            ...
            for pedlevel in range(8):
                if pedlevel < len(pedlevs):  
                    ...
                    if not blockdata.empty:
                        ...
                        result_fit = ps.psignifit(tofit, options)
                        allthresh[i, cond - 1, pedlevel] = result_fit['Fit'][0] 
                        allslope[i, cond - 1, pedlevel] = 10.3 / (result_fit['Fit'][1] / (norm.ppf(1 - 0.05) - norm.ppf(0.05)))
...
if r.doanalysis:
    raw_path = "local/rawEEGdata/"   #load data, you can download raw data from 
    nsubjs = 31
    ...
    # loop through each individual data, calculate the spectra data and amplitude at 6 consitons and 5 intensity levels
    for p in range(0,nsubjs):
        ...
        y_26Hz = []
        y_23Hz = []
        selectE = [4, 8, 9, 34, 35, 38] ##########  select electrdes:F1, F2, FZ, FC1, FC2, FCZ
            ...
            for l in range(len(levellist)):
                ...
                for electrode in range(s[0]):
                    ...
                    if electrode in selectE:
                        ...
                        spectra[idx, c, l, :] = np.abs(spec)  # store all frequencies from 6 electrdes
    ...
    np.savez('local/processedEEGdata.npz', allsubjsF1=allsubjsF1,allsubjsF2=allsubjsF2,selectSpec=selectSpec)
...
if r.plotfigures:
    np.load('local/processedEEGdata.npz')
    ...
    subjsF1 = np.zeros((31, 61, len(condlist), len(levellist)))
    subjsF2 = np.zeros((31, 61, len(condlist), len(levellist)))
    ...
    for el in range(61):
        for cond in range(6):
            for lev in range(5):
                ...
                for subj in range(31):
                    if subj < len(filtered_index):
                        subjsF2[subj, el, cond, lev] = temp[filtered_index[subj]]
                    else:
                        subjsF2[subj, el, cond, lev] = np.nan
...
if r.fitmodels:
    ...
    collist = ['red', 'blue', 'orange', 'darkgreen']
    ...
    for cond in range(4):
        plt.scatter(pedcontrasts, meanthresh[cond, :], color=collist[cond], marker='o', label=condition[cond])
        plt.errorbar(pedcontrasts, meanthresh[cond, :], yerr=SEthresh[cond, :], fmt='o', color=collist[cond])
        plt.plot(pedlist,allpreddB[cond], color=collist[cond], lw=2 )
    
    plt.xlabel('Pedestal contrast (%)',fontsize=20 )
    plt.ylabel('Threshold (%)', fontsize=20)
    plt.title('Linear summation model', fontsize=20)
    plt.xlim(-12,31)
    plt.ylim(-12,31)
    plt.yticks([-12,-6,0,6,12,18,24,30],[0.25, 0.5, 1, 2, 4, 8, 16, 32],fontsize = 15)
    plt.xticks([-12,-6,0,6,12,18,24,30],[0, 0.5, 1, 2, 4, 8, 16, 32] ,fontsize = 15  )
    ...
    plt.plot([-12, 30], [-12, 30], linestyle="--")
    ...
    for cond in range(4):
        plt.scatter(pedcontrasts, meanthresh[cond, :], color=collist[cond], marker='o', label=condition[cond])
        plt.errorbar(pedcontrasts, meanthresh[cond, :], yerr=SEthresh[cond, :], fmt='o', color=collist[cond])
        plt.plot(pedlist,allpreddB2[cond], color=collist[cond], lw=2 )
    
    plt.xlabel('Pedestal contrast (%)',fontsize=20 )
    plt.ylabel('Threshold (%)', fontsize=20)
    plt.title('Minkowski summation model', fontsize=20)
    plt.xlim(-12,31)
    plt.ylim(-12,31)
    plt.yticks([-12,-6,0,6,12,18,24,30],[0.25, 0.5, 1, 2, 4, 8, 16, 32],fontsize = 15)
    plt.xticks([-12,-6,0,6,12,18,24,30],[0, 0.5, 1, 2, 4, 8, 16, 32] ,fontsize = 15  )
    ...
    plt.savefig('Figures/dipperfit.pdf')
...
if r.fitmodels:
    ...
    #
        ...
        responses = np.array([
            # 26 Hz
            get_twoStage_model1(contrastsC, 0, 0, 0, p ),  # Pentadactyl_resp
            ...
            get_twoStage_model1(0, 0, contrastsC, 0, p),  # crossPentadactyl_resp
            ...
            ## 23 Hz
            get_twoStage_model2(contrastsC, 0, 0, 0, p ),  # Pentadactyl_resp
            ...
            get_twoStage_model2(0, 0, contrastsC, 0, p),  # crossPentadactyl_resp
    ...
    for i in range(ntotalsimplexfits):
      result = runfitting()
      print(result)
      allparams[i] = result
    ...
    print("model 1 rms:", modelRMS)
     ...
     ### Generate prediction responses  #12*5
    ...
    legend_elements = []
    linestyles = ['b--', 'r--', 'g--', 'y--', 'm--', 'k--', 'b--', 'r--', 'g--', 'y--', 'm--', 'k--']
    markerfacecolor = ['b','r', 'g', 'y','m', 'k', 'b','r', 'g', 'y','m', 'k']
    markers = ['o', 's', '^', 'o', 's', '^', 'o', 's', '^', 'o', 's', '^']
    
    # Plot fitting result of 26 Hz
    for i, linestyle, marker in zip(range(6), linestyles, markers):   # First 6 raws represent data at 26Hz
        plt.scatter(contrastsdB, mean[i,:], marker=markers[i], facecolors=markerfacecolor[i])
        plt.errorbar(contrastsdB, mean[i,:], yerr=SE[i,:], fmt='o',  color=markerfacecolor[i])
        plt.plot(contrastsdB, allresp[i], linestyle, label=conditions[i])
        legend_elements.append(Line2D([0], [0], color=linestyle[0], linestyle=linestyle[1:], marker=markers[i], label=conditions[i]))
    
    plt.title("26 Hz", fontsize = 20)
    plt.xlabel('Intensity level (%)',fontsize = 20)
    plt.xticks(contrastsdB,[0, 8, 16, 32, 64],fontsize = 15)
    plt.yticks(fontsize = 15)
    plt.ylabel('Model response',fontsize = 20)
    plt.legend(handles=legend_elements, bbox_to_anchor=(1.02, 1), loc='upper left', borderaxespad=0)
    plt.savefig('Figures/EEGfitA.pdf')
    
    # Plot fitting result of 23 Hz
    legend_elements = []
    for i, linestyle, marker in zip(range(6,12), linestyles, markers):     # Last 6 raws represent data at 23Hz
        plt.scatter(contrastsdB, mean[i,:], marker=markers[i], facecolors=markerfacecolor[i-6])
        plt.errorbar(contrastsdB, mean[i,:], yerr=SE[i,:], fmt='o',  color=markerfacecolor[i-6])
        plt.plot(contrastsdB, allresp[i], linestyle, label=conditions[i-6])
        legend_elements.append(Line2D([0], [0], color=linestyle[0], linestyle=linestyle[1:], marker=markers[i], label=conditions[i-6]))
    
    plt.title("23 Hz", fontsize = 20)
    plt.xlabel('Intensity level (%)',fontsize = 20)
    plt.xticks(contrastsdB,[0, 8, 16, 32, 64],fontsize = 15 )
    plt.yticks(fontsize = 15 )
    plt.ylabel('Model response',fontsize = 20)
    plt.legend(handles=legend_elements, bbox_to_anchor=(1.02, 1), loc='upper left', borderaxespad=0)
    plt.savefig('Figures/EEGfitB.pdf')
